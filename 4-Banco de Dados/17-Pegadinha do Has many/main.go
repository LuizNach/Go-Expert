...
func main() {
	...
	// Detalhe que agora ensina é carregar campos encadeados no Preload
	// Detalhe que coloquei o debug para entender as queries que o gorm faz
	err = db.Debug().Model(&Category{}).Preload("Products.Category").Preload("Products.SerialNumber").Find(&categories).Error
	if err != nil {
